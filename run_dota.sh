...
# ------------------------------- Base Pre-train ---------------------------------- #
python3 main.py --num-gpus 2 --config-file configs/dota/defrcn_det_r101_base.yaml     \
...
# BASE_WEIGHT=/home/aeen/fewshot/OBBDetection/work_dirs/faster_rcnn_orpn_r101_fpn_1x_dota10_with_novel/model_reset_remove.pth
# ------------------------------ Novel Fine-tuning -------------------------------- #
...
do
    ...
    do
        for seed in 0
        do
            python3 tools/create_config.py --dataset dota --config_root configs/dota \
                --shot ${shot} --seed ${seed} --setting 'fsod'
            CONFIG_PATH=configs/dota/defrcn_fsod_r101_novel_${shot}shot_seed${seed}.yaml
...
python3 tools/model_surgery.py --dataset dota --method randinit                        \
    --src-path ${SAVEDIR}/defrcn_det_r101_base/model_final.pth                         \
    --save-dir ${SAVEDIR}/defrcn_det_r101_base
'''
'''
...
do
    ...
    do
        ...
        CONFIG_PATH=configs/dota/defrcn_gfsod_r101_novel_${shot}shot_seed${seed}.yaml
...
do
    ...
    do
        python3 tools/create_config.py --dataset dota1024 --config_root configs/dota     \
            --shot ${shot} --seed ${seed} --setting 'fsod'
        CONFIG_PATH=configs/dota/defrcn_fsod_r101_novel_${shot}shot_seed${seed}.yaml
