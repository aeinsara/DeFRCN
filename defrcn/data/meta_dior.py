...
def load_dior_json(json_file, image_root, metadata, dataset_name):
    is_shots = "shot" in dataset_name  # few-shot
    if is_shots:
        imgid2info = {}
        shot = dataset_name.split('_')[-2].split('shot')[0]
        seed = int(dataset_name.split('_seed')[-1])
        split_dir = os.path.join('/storage/aeen/fewshot/Datasets/dior', 'diorsplit', 'seed{}'.format(seed))
...
def register_meta_dior(name, metadata, imgdir, annofile):
    DatasetCatalog.register(
        name,
        lambda: load_dior_json(annofile, imgdir, metadata, name),
    )
